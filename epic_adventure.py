...
MARCUS_ROCK_ENCOUNTER = EncounterConfig(
    ...
    intro_text=(
        "\n\tYou are sitting around a campfire, just living your best wormy acetomenotistic life with your best friend Robert. You notice something is off but you can't put your freakishly long finger on it. After a brief moment of hypervigilance, something catches your big ol' eye. There's something about that rock. That rock right there. It's... It's... IT'S PISSING YOU OFF! You tell Robert but he is of little help. You are left with no other choice than to give that stupid smug little stupid smug rock a piece of your mind.\n"
    ),
    victory_text=(
...
def describe_crossroad(
    player: Player,
    encounters: Dict[str, EncounterConfig],
...
def build_direction_prompt(encounters: Dict[str, EncounterConfig]) -> str:
    options = " / ".join(direction.title() for direction in encounters)
    return f"Which direction will you choose? ({options})\n> "


def enemy_choose_action(enemy: Enemy, enemy_health: int) -> str:
    ...
    while enemy_health > 0 and player.health > 0:
        ...
        if choice == "1":
            ...
            if enemy_action == "attack":
                ...
                print(f"{player.name} attacked and dealt {player_damage} damage!")
                ...
                print(f"The {enemy.name} attacked back and dealt {enemy_damage} damage!")
                ...
                print(f"{player.name} attacked for {player_damage} damage!")
                print(
                    f"The {enemy.name} raised its guard and blocked most of it! "
                    f"Only took {reduced_damage} damage."
                )
                print(f"Its counterattack dealt {counter_damage} damage to you!")
            ...
            if enemy_action == "attack":
                ...
                print(f"{player.name} and the {enemy.name} both brace for impact!")
                print(
                    "You circle each other warily. Both take 2 damage from exhaustion."
                )
                player.health -= 2
                enemy_health -= 2
...
def main():
    ...
    *******************************************
    ...
    encounters: Dict[str, EncounterConfig] = dict(BASE_ENCOUNTERS)
    if is_marcus:
